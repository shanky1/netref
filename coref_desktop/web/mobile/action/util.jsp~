...
<%!
    ...
    static String CONTACT_IMAGE_PATH = "D:\\coref\\web\\mobile\\user_contact_images";
    static String PROFILE_IMAGE_PATH = "D:\\coref\\web\\mobile\\profile_images";
    ...
    final String sql_getAllFLofFriends = "SELECT rs.to_user_id, rs.to_contact_name, profession, expertise, experience, linkedin " +
            "    FROM relationship rs LEFT JOIN employee_details s " +
...
//                        freelancer_email = new String(freelancer_email_ba);
    ...
    final String sql_updateAskResponse = "update activities_responses set comments = ? where activity_id = ?  and recommended_by = ?";
    ...
    final String sql_getFLSkills = "select * from employee_details where user_id = ?";
    final String sql_checkFLSkills = "select * from employee_details where user_id = ?";
    final String sql_insertFLSkills = "insert into employee_details (expertise,linkedin, facebook, hr_consent, user_id) values (?, ?, ?, ?, ?)";
    final String sql_updateFLSkills = "update employee_details set  expertise = ?, linkedin = ?, facebook = ?, hr_consent = ? where user_id = ?";
...
//  displays logged in user fl's and friend's fl's
            ...
            String sql_searchFLs = "select t2.profession, t2.expertise, t2.experience, t2.linkedin, t2.from_user_id, t2.to_user_id, t2.advanced_relation_type, t2.email, t2.user_id, t2.fb_photo_path, t2.fb_user_id, t2.name from " +
                    ...
                    "from users u, relationship r left join employee_details s " +
                    ...
                    "UNION "+
                    "select s.profession, s.expertise, s.experience, s.linkedin, r.from_user_id, r.to_user_id, r.advanced_relation_type, u.email, u.user_id, u.fb_photo_path, u.fb_user_id, r.to_contact_name as name "+
                    "from employee_details s, relationship r, users u  " +
        ...
        try {
            ...
            while(rs.next()) {
                ...
                if(category.equalsIgnoreCase("asks") || category.equalsIgnoreCase("broadcast")) {
                    msg += loadActivitiesAsksString(con, activity_id, posted_by_photo, fl_name, comments, posted_on, posted_by, owner_id, fl_list,post_likes,post_dislikes,post_comments);
                }
            }
        } catch(Exception se) {
...
//    final String sql_getFLVectorList = "select rs.to_user_id, rs.to_contact_name from users u, relationship rs where u.user_id = rs.to_user_id and rs.from_user_id = ? and rs.advanced_relation_type = 4 and active_status = 1";
    ...
    final String sql_getFLVectorList = "select rs.to_user_id, rs.to_contact_name, sk.profession from users u, relationship rs, employee_details sk " +
...
//                    e.printStackTrace();
    ...
    public ArrayList loadFreelancers_AL(String from_user_id) {
        ...
        try {
            ...
            final String sql_loadfreelancers = "select t2.profession, t2.expertise,t2.experience, t2.linkedin, t2.from_user_id, t2.to_user_id, t2.facebook, t2.advanced_relation_type, t2.email, t2.user_id, t2.fb_photo_path, t2.fb_user_id, t2.to_contact_name from " +
                    "( " +

//            My Professionals

                    "SELECT s.profession, s.expertise, s.experience, s.linkedin, s.facebook, rs.from_user_id, rs.to_user_id, rs.advanced_relation_type, u.email, u.user_id, u.fb_photo_path, u.fb_user_id, rs.to_contact_name " +
            ...
            while (rs.next()) {
                ...
                try {
                    ...
                    if(linkedin.trim().length() <= 0 && expertise.trim().length() <= 0) {
                        continue;
                    }
    ...
    public ArrayList loadReferrals_AL(String from_user_id) {
        ...
        try {
            ...
            final String sql_getReferrals = "select contact_user_id from contact_referrals where from_user_id = ? and active_status = 1";

            ps1 = getPs(con, sql_getReferrals);
            ps1.setString(1, from_user_id);

            rs = ps1.executeQuery();

            while (rs.next()) {
                int contact_user_id = rs.getInt("contact_user_id");

                String sql_getReferralDetails = "select r.to_user_id, r.to_contact_name, ed.expertise, ed.linkedin, ed.facebook from employee_details ed, relationship r " +
                        "where ed.user_id = r.to_user_id and r.active_status = 1 and r.from_user_id = ? and r.to_user_id = ?";

                ps2 = getPs(con, sql_getReferralDetails);
                ps2.setString(1, from_user_id);
                ps2.setInt(2, contact_user_id);
                ...
                if(rs2.next()) {
                    byte[] referral_name_enc = rs2.getBytes("to_contact_name");
                    byte[] referral_name_ba = processDecrypt(referral_name_enc);
                    String referral_name = new String(referral_name_ba);

                    int referral_user_id = rs2.getInt("to_user_id");
                    expertise = rs2.getString("expertise");
                    linkedin = rs2.getString("linkedin");
                    ...
                    hm.put("referral_userid", referral_user_id);
                    hm.put("referral_name", referral_name);
                    hm.put("expertise", expertise);
                    hm.put("linkedin", linkedin);
                    hm.put("facebook", facebook);
    ...
    public ArrayList loadEMPs_AL(String from_user_id) {
        ...
        try {
            ...
            String sql_getEMPs = "select uem.employee_id from users u, user_employee_mapping uem where u.user_id = uem.user_id and u.user_id = ? and uem.active_status = 1";

            ps1 = getPs(con, sql_getEMPs);
            ps1.setString(1, from_user_id);
            ...
            while (rs.next()) {
                ...
                String sql_getEMPDetails = "select r.to_user_id, r.to_contact_name from relationship r where r.from_user_id = ? and r.to_user_id = ?";

                ps2 = getPs(con, sql_getEMPDetails);
                ps2.setString(1, from_user_id);
                ps2.setInt(2, emp_id);
                ...
                if(rs2.next()) {
                    byte[] emp_name_enc = rs2.getBytes("to_contact_name");
                    byte[] emp_name_ba = processDecrypt(emp_name_enc);
                    String emp_name = new String(emp_name_ba);

                    int to_user_id = rs2.getInt("to_user_id");

                    hm = new HashMap();
                    hm.put("emp_userid", to_user_id);
                    hm.put("emp_name", emp_name);
    ...
    final String sql_getGamificationdetails = "SELECT  COUNT(a.contact_user_id) AS  referals, b.name  FROM users b  LEFT JOIN   contact_referrals a ON   b.user_id   =  a.from_user_id     WHERE  b.user_id =?   GROUP BY  a.from_user_id";

    public String gamification(String user_id) {
        Connection con = null;
        PreparedStatement getFLList = null;
        ResultSet rs = null;
        String course_list = "";

        String referal_name = null;
        ...
        try {
            ...
            getFLList = getPs(con, sql_getGamificationdetails);
            getFLList.setString(1, user_id);
            ...
            if(rs != null){
                while (rs.next()) {
                    ...
                    try {
                        byte[] referal_name_enc = rs.getBytes("name");
                        byte[] referal_name_ba = processDecrypt(referal_name_enc);
                        referal_name = new String(referal_name_ba);


                        tr = gamigicationString(user_id, referal_name,referals);
    ...
    public String gamigicationString(String user_id, String referal_name,String referals) {


        String ret = "<dl style='word-wrap: break-word;padding:0px; width: 99%'>\n" +
                "                                        <dd class='pos-left clearfix' style='margin-bottom:0px'>\n" +
                "                                            <div class='events' style='margin-top:3px;line-height:1.0;background-color:#ffffff;  display:inline-block;'>" +
                ...
                "                                                        <h2 style='margin-top:5px;margin-bottom:3px;font-size:20px;margin-left:18px'> "+referal_name+" </h2>" +
                "                                                        <span class=\"step pull-left\" style='font-size:20px'>"+referals+"</span><h1 style=\"margin-top: 7%;margin-right: 16%;font-size:23px;margin-bottom: 10px\">Referred</h1>" +
                "                                                    </div>" +
                "                                                </div>" +
                "</div> <br><br><br><br><br>\n" +
                "                                        </dd>\n" +
    ...
    final String sql_getTopreferals = "SELECT COUNT(a.contact_user_id) AS  referals, b.name  FROM contact_referrals a, users b WHERE a.from_user_id= b.user_id GROUP BY a.from_user_id ORDER BY referals desc limit 3";

    public String topreferals() {
        Connection con = null;
        PreparedStatement getFLList = null;
        ResultSet rs = null;
        String course_list = "";
        ...
        try {
            con = getConnection();
            getFLList = getPs(con, sql_getTopreferals);
...
//                        freelancer_email = new String(freelancer_email_ba);
                        ...
                        tr = topreferalsString(referal_name,referals);
    ...
    public String topreferalsString(String referal_name,String referals) {
        int points = (Integer.parseInt(referals))*2;

        String ret = " " +
                "<dl style='word-wrap: break-word;padding:0px; width: 99%'>" +
                "                                        <dd class='pos-left clearfix' style='margin-bottom:0px'>" +
                "                                            <div class='events' style='margin-top:3px;line-height:1.0;background-color:#ffffff;  display:inline-block;'>" +
                "                                                <p class='pull-left' style='margin-left:-3px;margin-bottom: 0px'> <img class='img-circle' style='main-width:35px;max-width:45px;margin-bottom:10px' src=images/profile.jpg></p>" +
                "                                                <div class='events-body ' style='margiun-right:0px;'>" +
                "                                                    <div align='left' class='pull-left' style='width:50%;margin-bottom:2px'>" +
                "                                                        <h2 style='margin-top:12px;margin-bottom:3px;font-size:17px;margin-left:18px;'>"+referal_name+" </h2>" +
                "                                                    </div>" +
                "                                                    <div align='right' class='pull-left' style='width:20%;margin-bottom:2px'>" +
                "                                                        <span class=\"pull-left\" style=\"margin-top: 15px;font-size: 15px;color: #FF6666\" ><b>"+points+"</b> pts</span>" +
                "                                                    </div>" +
                "                                                    <div align='left' class='pull-right' style='width:30%;margin-bottom:2px'>" +
                "                                                        <span class=\"step pull-left\" style=\"background:#008000; margin-top: 10px; font-size: 18px;\" >"+referals+"</span>" +
                "                                                    </div>" +
                "                                                </div>" +
                "                                            </div>" +
    ...
    public String loadAskList(String user_id) {
        ...
        String sql_loadAskList = "select a.*,u.* ,(SELECT  COUNT(p.like_status) FROM post_likes p  WHERE   p.activity_id =  a.activity_id and  p.like_status='1'   GROUP BY p.activity_id) as post_likes," +
                "(SELECT  COUNT(p.like_status) FROM post_likes p  WHERE  p.activity_id =  a.activity_id and p.like_status='0'   GROUP BY p.activity_id) as post_dislikes," +
                "(SELECT  COUNT(q.activity_id) FROM activities_responses q WHERE  a.activity_id = q.activity_id  GROUP BY q.activity_id)  as post_comments  " +
                " from activities a,users u where posted_by = ? and a.posted_by= u.user_id and category = \"asks\" and status = 1 order by posted_on DESC";
        ...
        try {
            ...
            while(rs.next()) {
                ...
                try {
                    ...
                    msg += loadAskListString(con, user_id, activity_id, comments, posted_on, posted_by_photo, name, posted_by,post_likes,post_dislikes,post_comments);
                } catch (Exception e) {
                    System.out.println(new Date()+"\t "+e.getMessage());
                    e.printStackTrace();
                }
...
//            System.err.print(se.getMessage());
    ...
    public String loadpost(String user_id,String activity_id) {
        String comments = "";
        String posted_on = "";
        String posted_by_photo = "";
        String posted_by = "";
        ...
        String sql_loadpost = "select * from activities a,users u where activity_id = ? and a.posted_by= u.user_id and category = \"asks\" and status = 1";
        ...
        try {
            ...
            while(rs.next()) {
                ...
                try {
                    ...
                    msg += loadpoststring(con, user_id, activity_id, comments, posted_on, posted_by_photo, name, posted_by);
                } catch (Exception e) {
                    System.out.println(new Date()+"\t "+e.getMessage());
                    e.printStackTrace();
                }
...
//            System.err.print(se.getMessage());
    ...
    public String loadmypost(String user_id,String activity_id) {
        String comments = "";
        String posted_on = "";
        String posted_by_photo = "";
        String posted_by = "";
        ...
        String sql_loadmypost = "select * from activities a,users u where activity_id = ? and a.posted_by= u.user_id and category = \"asks\" and status = 1";

        try {
            con = getConnection();

            ps = getPs(con, sql_loadmypost);
            ps.setString(1, user_id);
            ps.setString(1, activity_id);
            rs = ps.executeQuery();
            ...
            while(rs.next()) {
                ...
                try {
                    ...
                    msg += loadmypoststring(con, user_id, activity_id, comments, posted_on, posted_by_photo, name, posted_by);
                } catch (Exception e) {
                    System.out.println(new Date()+"\t "+e.getMessage());
                    e.printStackTrace();
                }
...
//            System.err.print(se.getMessage());
    ...
    final String sql_getProfileDetails = "SELECT u.name as profile_name, u.profile_image_file_name, u.businessdetails_image_file_name, u.user_type, s.* " +
            "            FROM users u LEFT JOIN employee_details s " +
            "            ON u.user_id = s.user_id " +
            "            where u.user_id = ?";
    ...
    public ArrayList getProfileDetails_AL(String from_user_id, String lin_publicProfileUrl) {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                profile_facebook = rs.getString("facebook");
...
//                    e.printStackTrace();
                ...
                try {
                    ...
                    profile_facebook = profile_facebook != null ? profile_facebook : "";
                    ...
                    hm.put("profile_facebook", profile_facebook);
...
//            System.err.print(se.getMessage());
    ...
    public String loadActivitiesAsksString(Connection con, String activity_id, String posted_by_photo, String fl_name, String comments, String posted_on, String posted_by, String owner_id, Vector fl_list,String post_likes,String post_dislikes,String post_comments) {
        ...
        String ret = "" +
                ...
                "                   <h3 class='events-heading text-left' style='display: inline;font-size: 14px'>"+fl_name+"  </h3>" +
        //        "                   <span class='text-muted' style='font-size: 12px'>"+category_to_display+" </span><br>" +
                "                   <p class='text-right text-muted' style='font-size: 10px; margin: 0 0 0px;'>"+Date_str1+"</p>" +
                "               </div>" +
                "            </div>" +

                "               <div style='margin-bottom:20px;display:inline;'>" +
                "                        <button  style='padding:0px; margin-top: 5px;background-color:#ffffff;margin-bottom: 5px' class='btn btn-default btn-simple btn-md pull-left' rel='tooltip' title='Post Response' data-original-title='Post Response' type='button' data-toggle='modal' >" +
                "                                <input type='text' id='like_"+activity_id+"' style ='width: 12px;border:none' value= "+(post_likes != null && post_likes.trim().length() > 0 ? post_likes  : "0")+"></input>&nbsp;<i class='fa fa-thumbs-o-up' style='color:#5bc0de;font-size: 20px' onclick=\"post_like("+activity_id+" , "+post_likes+");\"></i> &nbsp;&nbsp;&nbsp; " +
                "                            </button> " +
                "                           <button style='padding:0px; margin-top: 5px;background-color:#ffffff;margin-bottom: 5px;margin-right:15px' class='btn btn-default btn-simple btn-md pull-left' rel='tooltip' title='Post Response' data-original-title='Post Response' type='button' data-toggle='modal' >  " +
                "                               <input type='text' id='dislike_"+activity_id+"' style ='width: 12px;border:none' value="+(post_dislikes != null && post_dislikes.trim().length() > 0 ? post_dislikes  : "0")+"></input><i class='fa fa-thumbs-o-down' style='color:#fa8072;font-size: 20px' onclick=\"post_dislike("+activity_id+" , "+post_dislikes+");\"></i> " +
                "                           </button> " +

                "                            <button  id='showaskresponses_"+activity_id+"'  onclick='showpost("+activity_id+");' style='padding:0px; margin-top: 5px;background-color:#ffffff;margin-bottom: 5px' class='btn btn-default btn-simple btn-md pull-right' type='button' data-toggle='modal'>" +
                "                                &nbsp;<i class='fa fa-arrow-circle-right' style='color:#5bc0de;font-size: 20px'></i>  " +
                "                           </button> " +
                "                             <button  style='padding:0px; margin-top: 5px;background-color:#ffffff;margin-bottom: 5px' class='btn btn-default btn-simple btn-md pull-right' type='button' data-toggle='modal'> " +
                "                                 <input type='text' id='like_"+activity_id+"' style ='width: 12px;border:none' value= "+(post_comments != null && post_comments.trim().length() > 0 ? post_comments  : "0")+"></input>&nbsp;<i class='fa fa-commenting-o' style='color:#F6BB42;font-size: 20px'></i> &nbsp;&nbsp; " +
                "                           </button>  " +
                "               </div>"+
                "           </div>" +
                "       </div>" +
...
*/
    ...
    public String loadAskListString(Connection con, String user_id, String activity_id, String comments, String posted_on, String posted_by_photo, String name, String posted_by,String post_likes,String post_dislikes,String post_comments) {
        ...
        String ret = " <dl style='margin-bottom:-2px;margin-top:0.5%;padding:0px; width: 99%;'>" +
                "           <dd class='pos-left clearfix' >" +
                "               <div class='events' style='margin-top:0px;display:inline;background-color:#ffefef;padding-right: 7px; box-shadow: 0.09em 0.09em 0.09em 0.05em #888888;'>" +
                "            <div class='events-body' style='line-height:1.2'>"+
                "            <div>    " +
                "               <p align ='left' class='pull-left' style='font-size: 14px;line-height:1.3;display:inline;margin-bottom:0px;margin-top:2px;margin-left:3px'>" +
                                      comments+"" +
        ...
        "           <div class='events-right' style='margin-bottom:0%;margin-top: 1%'>" +
                ...
                "                   <p class='text-right text-muted' style='font-size: 10px; margin: 0 0 5px;'>"+Date_str+"</p>" +
                "               </div></div>" +
                ...
                "                                <input type='text' id='like_"+activity_id+"' style ='width: 12px;border:none;background-color:#ffefef' value= "+(post_likes != null && post_likes.trim().length() > 0 ? post_likes  : "0")+"></input>&nbsp;<i class='fa fa-thumbs-o-up' style='color:#5bc0de;font-size: 20px' onclick=\"post_like("+activity_id+" , "+post_likes+");\"></i> &nbsp;&nbsp;&nbsp; " +                "                            </button> " +
                ...
                "                               <input type='text' id='dislike_"+activity_id+"' style ='width: 12px;border:none;background-color:#ffefef' value="+(post_dislikes != null && post_dislikes.trim().length() > 0 ? post_dislikes  : "0")+"></input><i class='fa fa-thumbs-o-down' style='color:#fa8072;font-size: 20px' onclick=\"post_dislike("+activity_id+" , "+post_dislikes+");\"></i> " +
                "                           </button>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; " +
                ...
                "                                <input type='text'  style ='width: 12px;border:none;background-color:#ffefef' value= "+(post_comments != null && post_comments.trim().length() > 0 ? post_comments  : "0")+"></input>&nbsp;<i class='fa fa-commenting-o' style='color:#F6BB42;font-size: 20px'></i> &nbsp;&nbsp; " +
                "                           </button>  " +
    ...
    public String loadmypoststring(Connection con, String user_id, String activity_id, String comments, String posted_on, String posted_by_photo, String name, String posted_by) {
    ...
    public String loadpoststring(Connection con, String user_id, String activity_id, String comments, String posted_on, String posted_by_photo, String name, String posted_by) {
        ...
        String ret = " <dl style='margin-bottom:-2px;margin-top:0.5%;padding:0px; width: 99%;'>" +
                ...
                "            <div class='events-right' style='margin-bottom:0%;margin-top: 1%'>" +
                "               <div align='right' style='margin-left:9px'>" +
                "                   <h3 class='events-heading text-left' style='display: inline;font-size: 14px'>"+name+"  </h3>" +
                //        "                   <span class='text-muted' style='font-size: 12px'>"+category_to_display+" </span><br>" +
                "                   <p class='text-right text-muted' style='font-size: 10px; margin: 0 0 0px;'>"+Date_str+"</p>" +
                "               </div>" +
                "            </div>" +
                ...
                /*               "                      <div style='margin-bottom:20px;display:inline;'> " +
                                ...
                                "                               2&nbsp;<i class='fa fa-thumbs-o-down' style='color:#fa8072;font-size: 20px'></i> " +
                                "                           </button>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; " +
                             "                       <button  onclick=\"getpostdetailstodelete('"+activity_id+"');\" data-toggle='modal' type='button'  style='padding:0px; margin-top: 5px;background-color:#ffffff;margin-bottom: 5px' class='btn btn-default btn-simple btn-md pull-left' ><i class='fa fa-times' style='color:#ff6666;font-size: 20px;'></i></button>" +

                                "                            <button  id='showaskresponses_"+activity_id+"'  onclick='showmypost("+activity_id+");' style='padding:0px; margin-top: 5px;background-color:#ffffff;margin-bottom: 5px' class='btn btn-default btn-simple btn-md pull-right' type='button' data-toggle='modal'>" +
                                "                                &nbsp;<i class='fa fa-arrow-circle-right' style='color:#5bc0de;font-size: 20px'></i>  " +
                                "                           </button> " +
                                "                             <button  style='padding:0px; margin-top: 5px;background-color:#ffffff;margin-bottom: 5px' class='btn btn-default btn-simple btn-md pull-right' type='button' data-toggle='modal'> " +
                                "                                10&nbsp;<i class='fa fa-commenting-o' style='color:#F6BB42;font-size: 20px'></i> &nbsp;&nbsp; " +
...
//            System.err.print(new Date()+"\t "+se.getMessage());
    ...
    public int addOrUpdateFLskills(String expertise, String linkedin, String profile_facebook, String from_user_id, String hr_consent) {
        int status = 0;
        ...
        try {
            ...
            if(rs.next()) {
                ...
                ps.setString(3, profile_facebook);
                ps.setString(4, hr_consent);
                ps.setString(5, from_user_id);
    ...
    public int post_like_insert(String user_id, String activity_id,String like_status) {
        int post_ststus = 0;
        ...
        try {
            ...
            System.out.println(new Date()+"\t sql_checklikes -> ps: "+ps);
            rs = ps.executeQuery();
    ...
    final String sql_checkdislikes = "select * from post_likes where activity_id = ? and user_id = ?";
    final String sql_insertdislikeststus = "insert into post_likes (activity_id,user_id, like_status) values (?, ?, ?)";
    final String sql_updatedislikestatus = "update post_likes set  like_status = ? where  activity_id = ? and  user_id = ?";

    public int post_dislike_insert(String user_id, String activity_id,String like_status) {
        int post_dislike_ststus = 0;
        ...
        try {
            ...
            ps = getPs(con, sql_checkdislikes);
            ps.setString(1, activity_id);
            ps.setString(2, user_id);
            System.out.println(new Date()+"\t sql_checkdislikes -> ps: "+ps);
            rs = ps.executeQuery();

            if(rs.next()) {
                ps = getPs(con, sql_updatedislikestatus);
                ps.setString(1, like_status);
                ps.setString(2, activity_id);
                ps.setString(3, user_id);
                post_dislike_ststus = ps.executeUpdate();
            } else {
                ps = getPs(con, sql_insertdislikeststus);
                ps.setString(1, activity_id);
                ps.setString(2, user_id);
                ps.setString(3, like_status);
                post_dislike_ststus = ps.executeUpdate();
        ...
        return post_dislike_ststus;
    }

    final String sql_postSuggestionInNetwork = "insert into activities (posted_by, fl_userid, category, comments) values (?, ?, ?, ?)";
    final String sql_Suggestion = "select * from activities where fl_userid = ? and posted_by = ?";
    final String sql_updateSuggestion = "update activities set  comments = ? where  posted_by = ? and  fl_userid = ?";

    public int addsuggestions(String contact_user_id, String contactprofile_linkedin,String contactprofile_name,String from_user_id,String post_type,String fl_userid,String contactprofile_skills) {
        int status = 0;
        String linkedin_str1 = "<button class='btn btn-info btn-simple btn-fill   btn-sm' style='cursor: pointer;padding: 0px 5px' data-original-title='Linkedin profile' type='button' title='' rel='tooltip'  onclick=\"window.open('"+contactprofile_linkedin+"', '_blank')\"><i class='fa fa-linkedin'></i> </button>";
        String linkedin_str = (contactprofile_linkedin != null && contactprofile_linkedin.trim().length() > 0 ? linkedin_str1 : "");

        String post = " <a href=''>"+contactprofile_name+"</a> "+linkedin_str+" <br> Slills : "+contactprofile_skills+" ";
        PreparedStatement ps = null;
        ResultSet rs = null;
        Connection con = null;
        try {
            ...
            ps = getPs(con, sql_Suggestion);
            ps.setString(1, contact_user_id);
            ps.setString(2, from_user_id);
            System.out.println(new Date()+"\t sql_Suggestion -> ps: "+ps);
            rs = ps.executeQuery();

            if(rs.next()) {
                ps = getPs(con, sql_updateSuggestion);
                ps.setString(1, post);
                ps.setString(2, from_user_id);
                ps.setString(3, contact_user_id);
                System.out.println(new Date()+"\t sql_updateSuggestion -> ps: "+ps);
                 status = ps.executeUpdate();
            } else {
                ps = getPs(con, sql_postSuggestionInNetwork);
                ps.setString(1, from_user_id);
                ps.setString(2, contact_user_id);
                ps.setString(3, post_type);
                ps.setString(4, post);
                System.out.println(new Date()+"\t sql_postSuggestionInNetwork -> ps: "+ps);
                 status = ps.executeUpdate();
            }
        }
         catch(Exception se) {
    ...
    public String saveProfileDetails(String from_user_id, String profile_name,
                                     ...
                                     String profile_facebook, String hr_consent) {
        int name_status = updateProfileName(profile_name, from_user_id);
        int skills_status = addOrUpdateFLskills(profile_expertise, profile_linkedin, profile_facebook, from_user_id, hr_consent);
...
//            System.err.print(se.getMessage());
    ...
    public String postResponseToAsk(String user_id,String activity_id, String comments) {
        Connection con = null;
        PreparedStatement ps = null;
        ResultSet rs = null;
        int cnt = 0;
        ...
        try {
          ...
          ps = getPs(con, sql_postAskResponse);
            ...
            System.out.println("sql_checkAskResponse" +ps);
            ...
            if(rs.next()) {
               ...
               /* ps = getPs(con, sql_updateAskResponse);
                ...
                ps.setString(2, activity_id);
                ps.setString(3, user_id);
                System.out.println("sql_updateAskResponse" +ps);
                cnt = ps.executeUpdate();
            } else {*/
               /* ps = getPs(con, sql_postAskResponse);
                ...
                ps.setString(2, user_id);
                ps.setString(3, comments);
                System.out.println("sql_postAskResponse" +ps);
                cnt = ps.executeUpdate();*/
            }
...
//        System.out.println(new Date()+"\t getClientsForFL END");
    ...
    public ArrayList loadEMPReferrals_AL(String from_user_id, String emp_userid, String emp_name) {
        ArrayList emp_referral_list = new ArrayList();

        try {
            HashMap hm = new HashMap();
            hm.put("emp_ref_userid", 111);
            hm.put("emp_ref_name", "Makenna");

            emp_referral_list.add(hm);

            hm = new HashMap();
            hm.put("emp_ref_userid", 112);
            hm.put("emp_ref_name", "Kelsey");

            emp_referral_list.add(hm);

            hm = new HashMap();
            hm.put("emp_ref_userid", 113);
            hm.put("emp_ref_name", "Athena");

            emp_referral_list.add(hm);

            hm = new HashMap();
            hm.put("emp_ref_userid", 114);
            hm.put("emp_ref_name", "Carmen");

            emp_referral_list.add(hm);

            hm = new HashMap();
            hm.put("emp_ref_userid", 115);
            hm.put("emp_ref_name", "Charlie");

            emp_referral_list.add(hm);

            return emp_referral_list;
        } catch (Exception e) {
            System.out.println(new Date()+"\t "+e.getMessage());
            e.printStackTrace();
            return null;
...
//        System.out.println(new Date()+"\t getClientsForFL END");
    ...
    public String getAskResponsesList(Connection con, String recommended_by_name, String recommended_by_photo, String comments, String recommended_on,String activity_id) {
        ...
        String ret = " <div  class='text-left' style='max-width: 98%;margin-top: 0%;max-height: 200px;overflow: auto; overflow-x: hidden;overflow-y:scroll;'></div> " +
                ...
                "                            <div class='events' id='askresponse' style='background-color:#f1efef;'> " +
                "                               <div class='pull-left' style='margin-top: -1%;'> " +
                "                                   <img class='img-circle' onclick='showdetails();' style='max-width:30px;margin-top:1%;margin-right:7px' src='"+recommended_by_photo+"' class='events-object img-rounded'>" +
                "                               </div> " +
                "									 <div class='events-body' > "+
                "                                   <div align='left' style='line-height: 1.3;margin-top:-1%;margin-bottom:4px'> " +
                "                                       <h5 class='events-heading text-left dont-break-out' style='display: inline;font-size:13px;break-word;margin-left: 5px'>"+recommended_by_name+"  </h5>" +
                "                                       <span class='text-muted' style='font-size: 10px'>("+date_str_response+")</span><br>   " +
                "											<span style='font-size: 11px;word-wrap: break-word;margin-left: 5px'> "+comments+"  </span><br>"+
                "                               </div>" +
                "                               </div>" +
                "                            </div>" +
                "                        </dd>" +
    ...
    public boolean insertUserFBFriendsIfNotExist(String userId, String fb_user_id, List<User> friendsList) {
        ...
        try {
            ...
            for (User user1 : friendsList) {
                ...
                System.out.println(new Date()+"\t fb_friend_user_id: "+fb_friend_user_id+", fb_friend_user_name: "+fb_friend_user_name);
...
//                    System.out.println(new Date()+"\t name: "+name);
                    ...
                    if(name.equalsIgnoreCase("_uuid")) {                //ignoring the entry with _uuid, which is the entry for device id
                        continue;
                    }
                    byte[] name_enc = processEncrypt(name);
                    byte[] mobile_enc = processEncrypt(mobile);
...
//        test_numbers.put("+918951092157", "Ajay");
        ...
        String msg = "Thanks for choosing Coref. Verification code for your mobile is: "+verification_code+".";
    ...
    public String sendResponse_SMS(String from_name, String to_mobile, String comments) {
        ...
        System.out.println(new Date() + "\t sendResponse_SMS: " + msg);
...
//        params.add(new BasicNameValuePair("MediaUrl", "https://goo.gl/Njkmxu"));
    ...
    public String sendResponse_SMS_to_Professional(String loggedin_username, String activity_ownername, String to_mobile) {
        ...
        System.out.println(new Date()+"\t sendResponse_SMS_to_Professional: "+msg);
...
//        params.add(new BasicNameValuePair("MediaUrl", "https://goo.gl/Njkmxu"));
    ...
    public String inviteApp_SMS(String from_name, String to_mobile) {
        String msg = "Hi, "+(from_name != null && from_name.length() > 0 ? from_name:"Someone") +" invites you to try Coref. " +
                "Download the Android app from https://goo.gl/iHA7bG";
...
//        params.add(new BasicNameValuePair("MediaUrl", "https://goo.gl/Njkmxu"));
    ...
    public String remindAppInvite_SMS(String from_name, String to_mobile) {
        String msg = "Reminder: "+(from_name != null && from_name.length() > 0 ? from_name:"Someone") +" would like you to join on Coref. " +
                "Download the Android app from https://goo.gl/iHA7bG";
...
//        params.add(new BasicNameValuePair("MediaUrl", "https://goo.gl/Njkmxu"));
    ...
    public String reminder_SMS(String from_name, String to_mobile) {
        String msg = (from_name != null && from_name.length() > 0 ? from_name:"Someone") +" wants your feedback about Coref. " +
                "Download the Android app from https://goo.gl/iHA7bG";
...
//        params.add(new BasicNameValuePair("MediaUrl", "https://goo.gl/Njkmxu"));
    ...
    final static String sql_verifyMobileCodeHM = "SELECT u.user_id, vm.verification_code, u.user_type FROM verify_mobile vm, users u WHERE vm.user_id = u.user_id and u.mobile = ? order by create_time DESC";

    public ArrayList<HashMap<String, Integer>> verifyMobileCodeAL(String mobilenum, String verification_code) {
        Connection con = null;
        PreparedStatement verifyMobileCode = null, updateStatus = null;
        ResultSet rs = null;
        ...
        try {
            ...
            verifyMobileCode = getPs(con, sql_verifyMobileCodeHM);
            verifyMobileCode.setBytes(1, mobile_bytes);
    ...
    public int registerPhoneNumberIfNotExists(String country_code, String phonenum, String deviceIMEI) {
        ...
        //      -- if deviceid exists - do nothing
        //      -- else - update imei
        //-- else
        //      -- if deviceid exists - update mobile no
...
//        Check and insert if the phone registration doesn't exist
            ...
            if (rs.next()) {                                    //If mobileno already exists
                ...
                if (rs.next()) {                                //if deviceid exists - do nothing
                    //Do Nothing
                } else {
...
//                System.out.println("to_userId : "+to_userId+" invitation_status : "+invitation_status);
    ...
    String sql_add_employee_status = "select distinct (uem.employee_id), uem.active_status, r.rs_id from relationship r, user_employee_mapping uem where uem.employee_id = r.to_user_id and r.from_user_id = ?";
    public ArrayList getAddEmployeeStatus(String from_user_id) {
        Connection con = null;
        PreparedStatement getAIC = null;
        ResultSet rs = null;
        ArrayList employee_contact_list = new ArrayList();
        ...
        try {
            con = getConnection();
            getAIC = getPs(con, sql_add_employee_status);
            getAIC.setString(1, from_user_id);
            rs = getAIC.executeQuery();

            while (rs.next()) {
                employee_id = rs.getString("employee_id");
                active_status = rs.getInt("active_status");
                rs_id = rs.getString("rs_id");

                HashMap hm = new HashMap();
                hm.put("from_user_id", from_user_id);
                hm.put("employee_id", employee_id);
                hm.put("active_status", active_status);
                hm.put("rs_id", rs_id);

                employee_contact_list.add(hm);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }  finally {
...
//                            (pros_mobile != null && pros_mobile.trim().length() > 0 ? " Contact: "+pros_mobile +"<br>" : "") +
    ...
    /*-----------------------------get Employee referral details-----------------*/

    public String getEmpReferralDetails(String emp_userid, String emp_name) {
        String  ref_lin = "";
        String  ref_fb = "";
        String  ref_skills = "";
        String  ref_details = "";
        String  ref_image_file_name = "";

        ref_lin = "https://linkedin.com/******";
        ref_fb = "https://facebook.com/******";
        ref_skills = "Android, Cloud computing, Machine learning...";
        ref_image_file_name = "profile.jpg";

        ref_details = "<div id='contact_professional_info' style='display: none;'></div>" +
                "<div class='events pull-left' style=' word-wrap: break-word; width:100%; max-height: 140px; margin-top:3px;line-height:1.0;background-color:#ffffff; display:inline-block; border-radius:4px; overflow: auto; box-shadow: 0.09em 0.09em 0.09em 0.05em #888888; padding:5px 0px 5px 0px;'>" +
                " <p class='pull-left' style='margin-left:4px;margin-bottom: 0px'> " +
                " <img class='img-circle' style='main-width:25px;max-width:25px;margin-bottom:10px' src='profile_images/"+ref_image_file_name+"' onError='this.onerror=null;this.src=\"profile_images/profile.jpg\"' >" +
                " </p>" +
                " <div class='events-body ' style='margiun-right:0px;'>" +
                " <div align='left ' class='pull-left' style='margin-bottom:2px'>" +
                " <h4 class='' style='margin-bottom:2px;font-family:monospace;overflow:auto; font-size-adjust: 0.58;line-height:1.1;font-size:12px;margin-left:8px'> " +
                (emp_name != null && emp_name.trim().length() > 0 ? " Name: "+emp_name +"<br>" : "") +
                (ref_lin != null && ref_lin.trim().length() > 0 ? " Linkedin: "+ref_lin +"<br>" : "") +
                (ref_fb != null && ref_fb.trim().length() > 0 ? " Facebook: "+ref_fb +"<br>" : "") +
                (ref_skills != null && ref_skills.trim().length() > 0 ? " Skills: "+ref_skills +"<br>" : "") +
                " </h4>" +
                " </div> " +
                " </div>" +
                " </div>" +
...
//            getAIC.setLong(1, user_id);
    ...
    String sql_check_user_employee_map = "Select * from user_employee_mapping uem where uem.user_id = ? and uem.employee_id = ?";
    final String sql_insert_user_employee_map = "insert into user_employee_mapping (user_id, employee_id, active_status) values (?, ?, ?)";
    final String sql_update_user_employee_map = "update user_employee_mapping set active_status = ? where user_id = ? and employee_id = ?";

    public int addEmployeeForUser(int from_user_id, int employee_id, String rs_id, String from_country_code) {
        int status = 0;
        ...
        try {
            ...
            ps = getPs(con, sql_check_user_employee_map);
            ps.setInt(1, from_user_id);
            ps.setInt(2, employee_id);
            rs = ps.executeQuery();

            if(rs.next()) {
                System.out.println("Mapping already exist");
                //Mapping already exist
                ps = getPs(con, sql_update_user_employee_map);
                ps.setInt(1, 1);
                ps.setInt(2, from_user_id);
                ps.setInt(3, employee_id);

                status = ps.executeUpdate();
            } else {
                System.out.println("Mapping doesn't exist");
                //Mapping doesn't exist
                ps = getPs(con, sql_insert_user_employee_map);
                ps.setInt(1, from_user_id);
                ps.setInt(2, employee_id);
                ps.setInt(3, 1);
            ...
            addEmployeeInvite(con, from_user_id, employee_id, rs_id, from_country_code);
    ...
    public int removeEmployeeForUser(String from_user_id, String employee_id) {
        int status = 0;
    ...
    public void addEmployeeInvite(Connection con, int from_user_id, int contact_user_id, String rs_id, String from_country_code) {
        PreparedStatement ps = null;
        ResultSet rs = null;
        String from_name = "";
        String to_contact_name = "";
        ...
        try {
            ...
            if(flag1 && flag2) {
                System.out.println(new Date()+"\t addEmployeeInvite -> from_user_id: "+from_user_id+", contact_user_id: "+contact_user_id+", from_name: "+from_name+", to_contact_name: "+to_contact_name);
...
//                        inviteApp_SMS(from_name, test_mobile_number);
                ...
                int inv_id = addToAppInvitationTrack(con, from_user_id, fl_name_enc, contact_user_id, to_contact_name_enc, to_email_enc, to_mobile_enc, "AddEmployeeInvite", 1, 1);
            }
        } catch(Exception se) {
            System.out.println(new Date()+"\t appInvite -> could not send invitation, to_mobile: "+to_mobile+", from_user_id: "+from_user_id);
            se.printStackTrace();
