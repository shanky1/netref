...
//    on body load
    ...
    window.networkFriends1 = function () {
        if(device_type == "ios") {
        ...
        loadFLView();
        $("#contacts_page").show();                                 //hiding contacts tab by default on page load, that is on body load
        ...
        $("#professionals_page").hide();                            //showing professionals tab by default on page load, that is on body load
    };
...
//        $("#search_by_val_div").show();                            //showing search by pros input
    ...
    window.networkRererrals = function () {     //on body load, checking if the page redirected from the linkedin page
        // window.stop();                                              //stop loading the remaining page before calling new function. Is this correct way???
...
//                    alert("profilename_already_set");
            ...
            success:    function(profile_details_json) {
                ...
                for(var cnt = 0; cnt < profile_details_arr.length; cnt++) {
                    try {
                        ...
                        console.log("user_type: "+user_type+", hr_consent: "+hr_consent);
    ...
    window.saveContactProfileDetailsAndRefer = function(contact_user_id) {
        ...
        if(contactprofile_linkedin == null || contactprofile_linkedin == "") {
            if (contactprofile_skills == null || contactprofile_skills == ""){
                $("#contactprofile_details_status_failed").html("<div class='alert alert-danger' style='padding: 20px 0px 10px 10px;'>Please enter linkedin or skills<a href='#' class='custom_close' data-dismiss='alert' aria-label='close'>&times;</a>");
        ...
        $.ajax({
            ...
            success:    function(msg) {
                ...
                if(msg == "success") {
                    getsuggestion(contact_user_id,contactprofile_linkedin,contactprofile_name,contactprofile_skills);
    ...
    function getsuggestion(contact_user_id,contactprofile_linkedin,contactprofile_name,contactprofile_skills) {
...
function getProfileImage()  {
    ...
    if(device_type == "android") {
        ...
        $("#profile_details_status_success").html("<div class='alert alert-info'>To Be Done...<a href='#' class='custom_close' data-dismiss='alert' aria-label='close'>&times;</a>");
        $("#profile_details_status_success").show();
    }
}
...
//                console.log(new Date()+"\t Got the response: "+professional_list_json.length+", profession_list_json: "+professional_list_json);
                ...
                for(var cnt = 0; cnt < professional_arr.length; cnt++) {
                    try {
                        ...
                        var facebook = professional_arr[cnt].facebook;

                        var row_html = getProfessionalRowHTML(fl_userid, fl_name, profession, expertise, experience, linkedin, fb_photo_path, fb_user_id, facebook);
    ...
    window.loadReferralsView = function() {
        $("#search_results").val('');
        $("#load_referrals_loading").show();
        $.ajax({
            type:         "post",
...
//                console.log(new Date()+"\t Got the response: "+referral_list_json.length+", profession_list_json: "+referral_list_json);
    ...
    function getProfessionalRowHTML(fl_userid, fl_name, profession, expertise, experience, linkedin, fb_photo_path, fb_user_id, facebook) {
    ...
    window.get_search_results_fn = function() {
        ...
        if(professional_arr == null || professional_arr === undefined) {
            ...
            console.log("No results found for: "+typed_string);
        } else {
            for(var cnt = 0; cnt < professional_arr.length; cnt++) {
                try {
                    var fl_userid = professional_arr[cnt].fl_userid;
    ...
    window.showRererralsView = function() {
        $('#search_by_val').val('');
        $('#showfriendfreelancers').hide();
        $('#friendsview_table').hide();
        $('#prosclients').hide();
...
//            "                       <br>Start adding contacts." +
    ...
    function getRowHTML(from_user_id, contact_user_id, name, mobile, rs_id, contact_index, img_name_withpath) {
        ...
        row_html += "<dl class = 'contact_list' name = '"+name+"' id='"+contact_user_id+"' style='padding:0px; width: 98%'>" +
            ...
            "                               <h2 id='"+contact_user_id+"_contactname' style='margin-top:5px;margin-bottom:2px;font-size:15px;display:inline;'>"+name+"</h2>"+
            ...
            "                               </h2>" +
            "                            </div>" +
            "                            <div class='pull-left' style='width:10%;background-color:#ffffff;'> " +

            /*  "                               <button data-toggle='modal' style='padding:15px 10px 0px 10px; background-color:#ffffff' type='button' id='showprofile_"+contact_user_id+"' class='btn btn-primary btn-lg text-center'  onclick=\"openContactProfilePage('"+name+"',"+contact_user_id+");\">" +
             "                               <i class='fa fa-share-square' style='color: #ff6666;font-size:30px'></i>" +
             "                              </button> " +*/
            "                           </div>" +
            "                        </div>" +
            "                 </div>" +
