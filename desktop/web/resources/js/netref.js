...
function getNonotificationsHTML() {
    var no_notifications_html = "<dl id='no_pros_id' style='padding:2px'>  " +
        "   <dd class='pos-left clearfix'>" +
        "       <div class='events' style='margin-top: 2%; box-shadow: 0.09em 0.09em 0.09em 0.05em  #888888; background-color: #f2dede; color: #a94442;'>" +
        "           <div class='events-body'>" +
        "               <div style='margin-top: 10px; margin-bottom: 10px;'>"+
...
//    Currently not used?
    ...
    window.loadnotifications = function () {
        ...
        $('#mynetwork').hide();
        $('#network').hide();
        $("#networkfeed_page").hide();                               //showing networkfeed tab

        $("#networkfeed_post_div").hide();                            //showing network feed post input
        $("#notifications_page").show();                            //showing network feed post input
        $("#notificatis_view").show();                            //showing network feed post input
        $("#user_notifications").show();                            //showing network feed post input
        $("#profile").click();
    };
...
//                    alert("profilename_already_set");
            ...
            success:    function(profile_details_json) {
                ...
                for(var cnt = 0; cnt < profile_details_arr.length; cnt++) {
                    try {
                        ...
                        var profile_name = profile_details_arr[cnt].profile_name;
                        var profile_image_file_name = profile_details_arr[cnt].profile_image_file_name;
                        var businessdetails_image_file_name = profile_details_arr[cnt].businessdetails_image_file_name;
                        ...
                        if(businessdetails_image_file_name == null || businessdetails_image_file_name == "" || businessdetails_image_file_name == "Not Avilable") {
                            $("#business_details_image").attr("src","profile_images/profile.jpg");
                        } else {
                            $("#business_details_image").attr("src","profile_images/"+from_user_id+"/"+businessdetails_image_file_name);
                        }
            ...
            success:    function(profile_details_json) {
                ...
                for(var cnt = 0; cnt < profile_details_arr.length; cnt++) {
                    try {
                        ...
                        var contactprofile_expertise = profile_details_arr[cnt].profile_expertise;
                        ...
                        $("#contactprofile_expertise").val(contactprofile_expertise);
                        ...
                        $("#contactprofile_save").attr("onclick","saveContactProfileDetails('"+contact_user_id+"')");
    ...
    window.saveContactProfileDetails = function(contact_user_id) {
        var contactprofile_profession = $('#contactprofile_profession').val().trim();

        if(contactprofile_profession == null || contactprofile_profession == "") {
            $("#contactprofile_details_status_failed").html("<div class='alert alert-danger' style='padding: 20px 0px 10px 10px;'>Profession cannot be empty<a href='#' class='custom_close' data-dismiss='alert' aria-label='close'>&times;</a>");
            $("#contactprofile_details_status_failed").show();
            $('#contactprofile_profession').css({
                "border-color": "red"
            });
            $("#contactprofile_profession").focus();
        ...
        $.ajax({
            type:         "post",
            url:         "action/save_contactprofile_details.jsp",
            data:         "contactprofile_profession="+encodeURIComponent(contactprofile_profession)+"&contact_user_id="+contact_user_id,
            ...
            success:    function(msg) {
                ...
                if(msg == "success") {
                    $("#contactprofile_details_status_success").html("<div class='alert alert-success' style='padding: 20px 0px 10px 10px;'>Successfully saved the profession<a href='#' class='custom_close' data-dismiss='alert' aria-label='close'>&times;</a>");
                    $("#contactprofile_details_status_success").show();
                    $('#contactprofile_profession').css({
                        "border-color": ""
                    });
                } else {
                    $("#contactprofile_details_status_failed").html("<div class='alert alert-danger' style='padding: 20px 0px 10px 10px;'>Could not save the profession<a href='#' class='custom_close' data-dismiss='alert' aria-label='close'>&times;</a>");
                    $("#contactprofile_details_status_failed").show();
                }
            }
        });
...
function getBusinessDetailsImage()  {
    $("#profile_details_status_failed").hide();
    $("#profile_details_status_success").hide();

    //pick the business details image from the respective device
    if(device_type == "android") {
        try {
            webapp.getBusinessDetailsImage();
        } catch (err) {
//            console.log(new Date()+"\t Error while picking business details image from android: "+err);
            $("#profile_details_status_failed").html("<div class='alert alert-danger'>Could not get the business details image. Please update the APP, if required<a href='#' class='custom_close' data-dismiss='alert' aria-label='close'>&times;</a>");
            $("#profile_details_status_failed").show();
        }
    } else {
        $("#profile_details_status_failed").html("<div class='alert alert-info'>To Be Done...<a href='#' class='custom_close' data-dismiss='alert' aria-label='close'>&times;</a>");
...
//                    $("#search_results").focus();
    ...
    window.notifications_view = function() {
        $('#notifications').html('');
        $.ajax({
            type:         "post",
            url:         "action/load_notifications.jsp",

            success:    function(notification_list_json) {
                notification_list_json = escape(notification_list_json).replace(/%0A/g, "");
                notification_list_json = notification_list_json.replace(/%0D/g, "");
                notification_list_json = unescape(notification_list_json);

                if(notification_list_json != null && notification_list_json.indexOf("session_expired") >= 0) {
                    window.location = "mobileregister_nc.html";
                }

//                console.log(new Date()+"\t Got the response: "+professional_list_json.length+", profession_list_json: "+professional_list_json);

                notifications_arr = JSON.parse(notification_list_json);

                if(notifications_arr == null || notifications_arr.length <= 0) {

                    var no_notifications_html = getNonotificationsHTML();
                    $("#notifications").append(no_pros_html);

                    //No professional list found, do nothing
                    return;
                }

                var notifications_found = false;

                console.log(new Date()+"\t Number of notifications loading...: "+notifications_arr.length);

                for(var cnt = 0; cnt < notifications_arr.length; cnt++) {
                    try {
                        var fl_phone = notifications_arr[cnt].fl_phone;
                        var fl_name = notifications_arr[cnt].fl_name;
                        var fb_photo_path = notifications_arr[cnt].fb_photo_path;

                        var row_html = getnotificationsRowHTML(fl_phone, fl_name);

                        $("#notifications").append(row_html);
    ...
    function getnotificationsRowHTML(fl_phone, fl_name ,fb_photo_path) {
        fb_photo_path = (fb_photo_path != null && fb_photo_path.trim().length > 0 ? fb_photo_path : "images/profile.jpg");
        ...
        var row_html = "<dl style='word-wrap: break-word;padding:0px; width: 99%'>" +
            ...
            "                      <div class='events-body ' style='margiun-right:0px;'>" +
            "                           <div align='left' class='pull-left' style='width:90%;margin-bottom:2px'>   " +
            "                               <h2 style='margin-top:0px;margin-bottom:3px;font-size:15px;margin-left:8px'>"+(fl_name != null && fl_name.trim().length > 0 ? fl_name  : "N/A")+" &nbsp;("+(fl_phone != null && fl_phone.trim().length > 0 ? fl_phone  : "N/A")+") </h2>"+
            "                               <h2 class='text-muted' style='margin-top:0px;margin-bottom:3px;font-size:15px;margin-left:8px'>Intrested to work with you</h2>"+
...
$(document).ready(function() {
    ...
    window.getMobileContacts_FromServer = function() {
        $("#contact_search").val('');
        getMobileContacts();
    };

    window.getMobileContacts = function() {
        console.log(new Date()+"\t getMobileContacts called");

        setTimeout(function() {
            $.ajax({
                type:         "post",
                url:          "action/get_all_contacts_from_db_session.jsp",
                ...
                success:    function(contacts_all_json) {
                    ...
                    for(var cnt = 0; cnt < contacts_arr_all.length; cnt++) {
                        try {
                            ...
                            var row_html = getRowHTML(from_user_id, contact_user_id, name, mobile, rs_id, cnt, img_name_withpath);
...
//            "                       <br>Start adding contacts." +
    ...
    var dl_id;
    function getRowHTML(from_user_id, contact_user_id, name, mobile, rs_id, contact_index, img_name_withpath) {
        if(device_type == "android") {
        ...
        row_html += "<dl class = 'contact_list' name = '"+name+"' id='"+contact_user_id+"' style='padding:0px; width: 98%'>" +
            ...
            "                   </p>"+
            "                      <div class='events-body ' style='margiun-right:0px;'>" +
            "                           <div align='left' class='pull-left' style='width:80%;margin-bottom:2px; margin-left: 6px;'>   " +
            ...
            "                                   </button>" +
            "                                   <button id='invite_"+rs_id+"' data-toggle='modal' type='button' class='btn btn-primary btn-lg  text-center' style='padding: 8px 10px 0px 10px; background-color:#ffffff;margin-top: 1%' onclick=\"sendAPPInvite('"+rs_id+"', '"+contact_user_id+"');\">" +
            "                                       <i class='fa fa-user-plus' style='color: #22A7F0'></i>" +
            "                                   </button>" +
            "                                   <button id='invitesuccess_"+rs_id+"' data-toggle='modal' type='button' class='btn btn-primary btn-lg  text-center' style='padding: 8px 10px 0px 10px; background-color:#ffffff;margin-top: 1%; display:none' onclick=\"sendAPPInvite('"+rs_id+"', '"+contact_user_id+"');\">" +
            "                                       <i class='fa fa-user-plus' style='color: #12B812'></i>" +
            "                                   </button>" +
            "                               </h2>" +
            "                            </div>" +
            "                            <div class='pull-right' style='width:10%;background-color:#ffffff;'> " +
            "                               <button data-toggle='modal' type='button' id='showprofile_"+contact_user_id+"' class='btn btn-default btn-simple btn-lg pull-right' style='background-color:#ffffff;' onclick=\"openContactProfilePage('"+name+"',"+contact_user_id+");\">" +
            "                                   <i class='fa fa-angle-right' style='color:#808080;font-size:25px'></i>" +
            "                               </button>" +
            "                           </div>" +
            "                        </div>" +
...
//                    console.log("name: "+name+", typed_string: "+typed_string);
                        ...
                        var row_html = getRowHTML(from_user_id, contact_user_id, name, mobile, rs_id, cnt_all, img_name_withpath);
...
//                alert("Approval pending...");
    ...
    window.shareContactForm = function(contact_user_id) {
        ...
        $("#contact_page_status_info").html("<div class='alert alert-danger' style='padding: 20px 0px 10px 10px;'>Share - To be implemented<a href='#' class='custom_close' data-dismiss='alert' aria-label='close'>&times;</a>");
        $("#contact_page_status_info").show();
    };
    ...
    window.contactProfessional = function(professional_id, from_user_id) {
        var msg = "<div class='modal-dialog'>" +
            "            <div class='modal-content' style='max-width: 550px'>" +
            "               <div align='center' class='modal-header' style='background-color:#2C93FF;border-radius: 5px 5px 0px 0px; padding: 10px;'>" +
            "                   <div style='opacity: 0.8; color: white; display: inline; padding: 0px 10px 0px 10px; font-size: 20px;' class='pull-right' onclick=\"$('#send_notification').hide();\">&times;</div>" +
            "                   <h5 class='modal-title pull-center' style='margin-bottom: 0px;height:5px;color: white;display: inline;'>Request contact</h5>" +
            "               </div>" +
            "                <div class='modal-body'> " +
            "                   <p align='center' style='margin-bottom: 0px'>Are you sure you wish to send a request</p>" +
            "                   <div id='add_cledit_status' align='center' style='display:none'></div>" +
            "                   <div class='modal-footer' style='margin-top:-2%;display:inline; border-top: 0px;'>" +
            "                       <center>" +
            "                           <button id='fcm_id' class='btn btn-info' style='background-color:#2C93FF'  data-toggle='button' type='submit' onclick=\"add_notification("+professional_id+","+from_user_id+");\">Go ahead</button>&nbsp;&nbsp;" +
            "                           <button class='btn btn-secondary active' onclick=\"$('#send_notification').hide();\">Cancel</button>" +
            "                       </center>" +
            "                   </div>" +
            "               </div>" +
            "            </div>" +
            "       </div>";
        $("#send_notification").html(msg);
        $('#send_notification').show();
    };

    window.add_notification = function(professional_id, from_user_id) {

        $.ajax({
            type:         "post",
            url:         "action/add_notification.jsp",
            data:         "professional_id="+professional_id+"&from_user_id=" + from_user_id,
            ...
            success:    function(msg) {
                ...
                if(msg != null && msg.indexOf("session_expired") >= 0) {
                    ...
                    $('#contact_professional').attr("class","btn btn-success btn-sm");
                }
            }
        });
    };
